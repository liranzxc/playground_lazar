...
public class GeneratorService implements IGeneratorService{
	...
	public String generateValidationCode() {
		    ...
		    for (int i = 0; i < targetStringLength; i++) {
		    	...
		    	if(p < 0.33) {
		    		int randomLimitedInt = leftWordLimit + (int) 
		    				(random.nextFloat() * (rightWordLimit - leftWordLimit + 1));
		    		buffer.append((char) randomLimitedInt);
		    	}
		    	else if (p<0.66 && p>=0.33){
			        int randomLimitedInt = leftNumberLimit + (int) 
			  	          (random.nextFloat() * (rightNumberLimit - leftNumberLimit + 1));
			  	        buffer.append((char) randomLimitedInt);
		    	}
		    	else {
			        int randomLimitedInt = leftLowerLimit + (int) 
				  	          (random.nextFloat() * (rightLowerLimit - leftLowerLimit + 1));
				  	        buffer.append((char) randomLimitedInt);
		    	}
		    }
